...
describe "A movie" do
	...
	it "is displayed if released in the past" do
		movie = Movie.create(movie_attributes(released_on: 1.month.ago))

		expect(Movie.released).to include(movie)
	end

	it "is not displayed if released in the future" do
		movie = Movie.create(movie_attributes(released_on: 1.month.from_now))

		expect(Movie.released).not_to include(movie)
	end	

	it "returns released movies ordered by the most recently released movies first" do
		movie1 = Movie.create(movie_attributes(released_on: 3.month.ago))
	...
	it "accepts a properly formatted image file" do
	...
	it "rejects the movie if it is not in the approved list" do
		ratings = %w[R-14 R-16 R-18 R-21]
		ratings.each do |rating|
			movie = Movie.new(rating: rating)
			movie.valid?
	...
	it "calculates the average number of review stars" do
		...
		expect(movie.average_stars).to eq(3.0)
	end
end